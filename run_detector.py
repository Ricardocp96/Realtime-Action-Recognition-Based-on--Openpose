...
from flask import Flask, flash, redirect, request, render_template, send_file, jsonify,make_response,send_from_directory, url_for
...
##Configs 
UPLOAD_FOLDER = 'skeleton_data/images_info'
ALLOWED_EXTENSIONS = { 'png', 'jpg', 'jpeg'}
RESULT_FOLDER='skeleton_data/skeletons2_images'
path ="skeleton_data/skeletons2_images" + "00001.png"
app=Flask(__name__,    template_folder='./frontend/build')
 ## prevent CORS 
cors = CORS(app)
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
socketio = SocketIO(app)
#Verify type of file uploaded 
...
## Storage configs 
config ={
  "apiKey": "AIzaSyCWAaP8FzgtNUu4YGdK1AdJgiGRcBlnJ-8",
  "authDomain": "action-recignition.firebaseapp.com",
  "projectId": "action-recignition",
  "storageBucket": "action-recignition.appspot.com",
  "messagingSenderId": "320284850893",
  "appId": "1:320284850893:web:a703f418313f2db2778784",
  "measurementId": "G-2QNT8XWPQN",
  "databaseURL":" https://action-recignition-default-rtdb.firebaseio.com"
...
storage.child("Images/result.jpeg").put("actionimage.jpeg")
@app.route('/')
def index():
...
## Create the Mobile Endpoint
@app.route('/mobile', methods=['GET','POST'])
def receive_mobile_image():
    labels=[]
    my_detector = SkeletonDetector()
   
    # -- Load images
    if FROM_CUSTOM:
        images_loader = load_mobile_imageloader()
    ...
    # -- Classify action
    #Research inference Method 
    if DO_INFERENCE:
        classifier = ActionClassifier(
            ## TOD CHANGE TO USE A NEW MODEL 
            CURR_PATH + "../model/new_action_recognition.h5"
        )

    # -- Loop through all images
    ith_img = 1
    while ith_img <= images_loader.num_images:
        img, action_type = images_loader.load_data()
        image_disp = img.copy()
...
def receive_image():   
    ...
    # -- Load images
    if FROM_CUSTOM:
        images_loader = load_custom_imageloader()
    ...
    while ith_img <= images_loader.num_images:
        ...
        #return send_from_directory(RESULT_FOLDER, filename='00001.png', as_attachment=True)
...
SAVE_DETECTED_SKELETON_IMAGES_TO =  "static/"+data_idx+""""""
SAVE_IMAGES_INFO_TO =               "skeleton_data/images_info"+data_idx+".txt"

DO_INFERENCE =  True and FROM_CUSTOM
SAVE_RESULTANT_SKELETON_TO_TXT_AND_IMAGE = True and  FROM_CUSTOM
...
#Receive Http post  request 
     ...
     ## call the function to save directory 
...
##Mobile image Loader 
##Custom image loader
class load_mobile_imageloader(object):
    def __init__(self):
        self.num_images=9999999
        
    def load_data(self):

        #receive image from the React Native Mobile App 
        #      
         #receive image from the frontend
        imgbytes=request.get_data()
        with open ('actionimage.jpeg', 'wb') as out:
             out.write(imgbytes)
             img=cv2.imread('actionimage.jpeg')
...
##Custom image loader
class load_custom_imageloader(object):
    def __init__(self):
        self.num_images=9999999
    ...
    def load_data(self):
        ...
        if file and allowed_uploads(file.filename):
...
#implement a flask server here to receive a post request from the front end and get a response showing the results 
class ActionClassifier(object):
    ...
    def predict(self, skeleton):
        ...
        # input n dimentional array into the model with numpy
